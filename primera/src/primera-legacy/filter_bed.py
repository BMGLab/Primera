from dataclasses import dataclass
import pandas as pd
import argparse

def main():
    template_url = "https://genome.ucsc.edu/cgi-bin/hgPcr?hgsid=2900325362_8e48BzUFKDYcxxsPnAlLNmzHKyGA&org=Human&db=hg38&wp_target=genome&wp_f={f}&wp_r={r}&Submit=Submit&wp_size=300&wp_perfect=15&wp_good=15&boolshad.wp_flipReverse=0&wp_append=on&boolshad.wp_append=0"

    parser = argparse.ArgumentParser(description = "filterBed")

    parser.add_argument("-b","--bedFile",
                        help = "Path to the .bed file")

    parser.add_argument("-p","--primers",
                        help = "Path to primers file")

    parser.add_argument("-c","--chrs",
                        help = "Chromosomes you want to filter.")
    
    args = parser.parse_args()

    input_bed = args.bedFile
    input_primers = args.primers
    target_chrs = args.chrs

    if input_bed == None or input_primers == None or target_chrs == None:

        print("Arguments can't be empty!")
        exit()

    target_sorted = target_chrs.split(",")
    target_sorted.sort()

    primer_sequences = {}
    with open(input_primers) as f:
        for line in f:
            fields = line.strip().split('\t')
            if len(fields) >= 3:
                primer_id = fields[0]
                forward = fields[1]
                reverse = fields[2]
                primer_sequences[primer_id] = (forward, reverse)

    primer_pairs = {}
    with open(input_bed) as f:
        for line in f:
            fields = line.strip().split()
            if len(fields) < 4:
                continue
            key = fields[3] 
            chrom = fields[0]
            loc = fields[1]
            locEnd = fields[2]
            forward,reverse = primer_sequences[key]
            id_ = f"{key.split("_")[0]}_{key.split("_")[1]}"

            if key in primer_pairs:

                if chrom in primer_pairs[key].chrs:

                    primer_pairs[key].chrs[chrom].append((loc,locEnd))

                else:

                    primer_pairs[key].chrs[chrom] = [(loc,locEnd)]

                primer_pairs[key].chrs_Exact.append(chrom)

            else:
                newPrimerPair = Primer_Pair(id_, forward, reverse, {chrom: [(loc,locEnd)]}, [chrom])
                primer_pairs[key] = newPrimerPair

    cols = ["id","chrs","locs","locsEnd","first","forward","reverse","url","seg_ids","segs"]
    df = pd.DataFrame(columns = cols)


    for i in primer_pairs:
        pair = primer_pairs[i]
        chrs_sorted = pair.chrs_Exact
        chrs_sorted.sort()
        
        df_chrs = ""
        df_locs = ""
        df_locs_end = ""

        if chrs_sorted == target_sorted:

            for _chr in pair.chrs:

                for _c in pair.chrs[_chr]:

                    df_chrs += f",{_chr}"
                    
                    df_locs += f",{_c[0]}"

                    df_locs_end += f",{_c[1]}"


            first = df_locs.split(",")[1]
            
            # TODO: Just for faster tests, change the statement when finalizing the project.
            if 1 < 0 : 
                with open(f"{pair._id}_reversed.fa","r") as f:
                            headers = ""
                            segs = ""

                            for line in f:

                                if line.startswith(">"):

                                    headers += f",{line}"

                                else:

                                    segs += f",{line}"
            else:
                headers,segs = "N/A","N/A"
          
            url = template_url.format(f=pair.forward, r=pair.reverse)
            headers,segs = headers[1:],segs[1:]
            df.loc[len(df)] = [pair._id, df_chrs[1:], df_locs[1:], df_locs_end[1:], first ,pair.forward, pair.reverse, url, headers, segs]

    df.sort_values(by="first",inplace=True,ascending=True)

    newDfList = []

    threshold = 18
    num = int(df.iloc[0,4]) - 18 - 1

    for i,j in enumerate(df.iloc[:, 4]):
        j = int(j)
        if j - num >= threshold:
            num = j
            newDfList.append(df.iloc[i,:])
            
    df = df.drop('first', axis=1)

    df2 = pd.DataFrame(newDfList,columns=df.columns)

    df2.to_csv("results.tsv",sep="\t",header=False,index=False)
